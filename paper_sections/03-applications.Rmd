...
## 3.3 | Simulating tracks from `aniMotum` model fits

To illustrate how to simulate tracks from `aniMotum` model fits we use a juvenile harp seal (*Pagophilus groenlandicus*) tracked from the Gulf of St Lawrence, Canada, \citep[described in][]{Grecian:2022}. The data are temporally irregular Argos locations including error ellipse information. We fit the `crw` model using `fit_ssm` with a 4 ms^-1^ speed filter threshold (`vmax`) and a 12-h prediction interval (`time.step`).
...
# 
# set.seed(10000)
# trs <- aniMotum::sim_fit(fit,
#               what = "predicted",
#               reps = 100,
#               grad = grad,
#               beta = c(-350, -350))
# 
# trs_flt <- aniMotum::sim_filter(trs,
#                       keep = 0.25,
#                       flag = 2)
# 
# trs_flt_rr <- aniMotum::route_path(trs_flt,
#                                 map_scale = 50,
#                                 buffer = 20000)
...
plocs_sf <- aniMotum::grab(fit, "predicted", as_sf = TRUE) %>%
  sf::st_transform(crs = prj) %>%
  group_by(id) %>%
  summarise(do_union = FALSE, .groups = "drop") %>%
  sf::st_cast("MULTIPOINT")
