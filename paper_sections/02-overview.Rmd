...
There are a large number of optional arguments that can be specified in `fit_ssm`, and these are explained in the package documentation. We focus only the essential arguments here:  
...
Before using fitted or predicted locations, a `fit_ssm` model fit should be checked and visualized to confirm that the model adequately describes the data. There is no simple way to calculate residuals for latent variable models that have non-finite state-spaces and that may be nonlinear, but they can be computed based on iterative forecasts of the model \citep{Thygesen:2017}. The `osar` function computes one-step-ahead (prediction) residuals via the  `oneStepPredict` function from the `TMB` R package \citep{Kristensen:2016}. A set of residuals are calculated for the `x` and `y` values corresponding to the fitted values from the SSM. A generic `plot` (`plot.osar`) method provides an easy way to visualize the prediction residuals as time-series plots, quantile-quantile plots, or autocorrelation functions (Fig. \ref{fig:workflow}).
...
`fit_ssm` model fits can be mapped using the `map` function for single or multiple individuals. By default, `map` uses the coastline data from the `rnaturalearth` R package \citep{rnaturalearth} at medium or high resolution (Fig. \ref{fig:workflow}), but can also use tiled maps for finer-scale detail, via the `rosm` \citep{rosm} and `ggspatial` \citep{ggspatial} R packages. Mapping aesthetics (e.g., plot symbols, sizes, colours, fills) can be customized via the `aes` argument and use of the `aes_lst` function. See code in SI for examples.
