...
Simulation is also used frequently to infer habitat availability, i.e., a null model of population distribution in the absence of external drivers, in habitat utilization studies [@Raymond:2015;@Hindell:2020]. The `simfit` function extracts movement parameters from a `fG_ssm` fit object and simulates an arbitrary number of random tracks of the same duration from these parameters. The argument `cpf = TRUE` ensures that the simulated tracks start and end at the same location, thereby simulating a central place forager.
