...
```
The workflow for `foieGras` is deliberately simple, with many of the usual track data processing checks and formatting handled automatically. Here we outline the main aspects of the `foieGras` package. Further details on usage are provided in the package's help documentation and vignettes. The packages's main functions are listed in Table \ref{tab:func_table_kable}. 
...
## 2.4 | Model checking and visualization - `osar`, `plot`, `fmap`
...
As the SSMs implemented in `foieGras` have no information about potential barriers to animal movement, it is possible for locations to be estimated in implausible locations such as on land for entirely or mostly aquatic species. To overcome this, `foieGras` makes use of the `pathroutr` R package [@pathroutr] to re-route locations from land back to water. 


In principle, the reverse could be implemented for land-based species 
